...
The IITD within-dataset subject-disjoint result also supports a conservative claim. It is a mixed near-tie result: B6 slightly improves the average Rank-1, Rank-5, EER, and TAR@FAR=$10^{-2}$ metrics, but it loses Macro-F1 and strict TAR@FAR=$10^{-3}$. Because IITD is recorded as a single-session dataset in the manifest, it should be read as within-dataset secondary validation, not as cross-session evidence.
