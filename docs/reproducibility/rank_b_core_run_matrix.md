...
* **Tongji (Primary)**: Evaluated using the **development/test palm-class-disjoint cross-session protocol**.
* **IITD (Secondary)**: Evaluated using the **secondary palm-class-disjoint within-dataset validation** (no cross-session claims).
...
All split configurations audited below are verified to be strictly palm-class-disjoint at the manifest-field level between development (train ∪ val) and test (gallery ∪ probe ∪ support) subsets.
...
Manifest subject-field overlap is audited, but independently verified person-disjointness is not asserted.
