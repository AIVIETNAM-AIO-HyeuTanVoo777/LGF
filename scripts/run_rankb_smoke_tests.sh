#!/usr/bin/env bash
# run_rankb_smoke_tests.sh
# Smoke test to verify loading and evaluating a model checkpoint using the correct config.

set -e

echo "=== Running Tongji Smoke Test (M0 - ResNet18 + CE, S1->S2, Seed 42) ==="
...
echo "=== Running IITD Smoke Test (M1 - ResNet18 + CE + SupCon, Within, Seed 42) ==="
python scripts/eval_embedding.py \
